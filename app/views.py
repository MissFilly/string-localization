...
def ModifyStringsHandler(request):
    ...
    if request.method == 'POST':
        ...
        for key in keys:
            if request.POST.get(key) and request.POST.get(key) != CURRENT_VALUES[int(key)]:
                try:
                    String.objects.filter(translator=translator,
                                     id=int(key),
                                     frozen=False).update(text=request.POST.get(key))
                except String.DoesNotExist:
        ...
        strings_list = String.objects.filter(translator=translator, frozen=False)
        paginator = Paginator(strings_list, 2) # Show 15 strings per page
        page = request.GET.get('page')
        try:
            strings = paginator.page(page)
